...
- **代码评审后修复**（一轮 general-purpose reviewer，verdict: merge-with-fixes、无 Critical、Windows verbatim-move 确认无回退）：ProxyDriver.has_proxies + 全禁用降级告警(#3)、Mac Accessibility 运行时提示(#4)、KeyboardInterrupt 不罚代理(#7)、osascript TimeoutExpired 捕获(#9)、Windows 缺 pygetwindow 友好报错(#10)、代理 user:pass/socks 文档修正(#1/#6)、测试补 proxy_url 透传 launch_chrome(#11)。
- **MacDriver 真机验证后重写**（本机 Chrome+Brave 同时运行）：初版「AppleScript by app-name + URL 匹配 + maximize-to-desktop」真机失败——① 多实例下 by-name 寻址错实例 → 改 **System Events by popen.pid**；② `--window-position/--window-size` 不被遵守、窗口落副屏 → 改 **AX set position/size 归一主屏 1280×860**（`bounds of window of desktop` 多显示器返回并集，弃用 maximize）；③ `pkill -f 全路径` 因 `/var`→`/private/var` 匹配不上 → 改 **popen kill + pkill basename**。全链路（launch→find→归一→前台→kill+清理）真机验证通过，未触碰日常浏览器。**遗留**：点击分数需真机跑一次真实注册来校准。
